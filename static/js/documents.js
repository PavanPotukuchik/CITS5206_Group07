$(document).ready(function() {
    ...
    async function loadFiles(userId) {
        ...
        try {
            const response = await fetch(`https://127.0.0.1:8090/api/collections/files/records?filter=(user='${userId}')`, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${JSON.parse(authData).token}`
    ...
    function displayFile(file) {
        ...
        li.innerHTML = `
            ...
            <a href="https://127.0.0.1:8090/api/files/files/${file.id}/${file.file}" target="_blank" class="file-name">${file.filename}</a>
    ...
    async function deleteFile(fileId) {
        try {
            const response = await fetch(`https://127.0.0.1:8090/api/collections/files/records/${fileId}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${JSON.parse(authData).token}`
                }
    ...
    function downloadFile(fileId, fileName, originalName) {
        ...
        link.href = `https://127.0.0.1:8090/files/files/${fileId}/${fileName}`;
